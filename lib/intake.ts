...
export const services = {
  dress_me: { name: "Mia, Dress Me", price: 19, short: "Style what I already own.", uploads: true },
  build_my_look: { name: "Mia, Build My Look", price: 39, short: "Shop me a new look.", uploads: false },
  build_my_fits: { name: "Build My Fits", price: 49, short: "Build multiple outfits from my wardrobe.", uploads: true },
  closet_reset: { name: "Closet Reset", price: 99, short: "Review and refresh my wardrobe.", uploads: true },
} as const;
...
export const intakeSchema = z.object({
  ...
  serviceType: z.enum(serviceKeys),
  ...
  if (data.serviceType === "dress_me") requireFields([["dressFor", "What you need to dress for"]]);
  if (data.serviceType === "build_my_look") requireFields([["totalBudget", "Total budget"], ["wontWear", "Items you will not wear"]]);
  if (data.serviceType === "build_my_fits") requireFields([["lifestyle", "Lifestyle"], ["mostUsedPieces", "Most-used pieces"], ["strugglePieces", "Pieces you struggle to style"], ["openToPurchase", "Purchase preference"]]);
  if (data.serviceType === "closet_reset") requireFields([["typicalWeek", "Typical week"], ["workExpectations", "Work dress expectations"], ["weekendStyle", "Weekend style"], ["wardrobeProblems", "Wardrobe problems"], ["mostWorn", "Most-worn pieces"], ["rarelyWorn", "Rarely-worn pieces"], ["styleGoals", "Style goals"], ["openToGaps", "Wardrobe-gap preference"]]);
});
